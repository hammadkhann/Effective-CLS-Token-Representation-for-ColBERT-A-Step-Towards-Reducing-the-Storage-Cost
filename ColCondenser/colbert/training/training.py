...
from colbert.modeling.colcondenser import ColCondenser
...
def train(args):
    ...
    colcondenser = ColCondenser.from_pretrained('Luyu/condenser',  # <- 'bert-base-uncased',
    ...
    if args.checkpoint is not None:
        ...
        try:
            colcondenser.load_state_dict(checkpoint['model_state_dict'])
        except:
            print_message("[WARNING] Loading checkpoint with strict=False")
            colcondenser.load_state_dict(checkpoint['model_state_dict'], strict=False)

    if args.rank == 0:
        torch.distributed.barrier()

    colcondenser = colcondenser.to(DEVICE)
    colcondenser.train()

    if args.distributed:
        colcondenser = torch.nn.parallel.DistributedDataParallel(colcondenser, device_ids=[args.rank],
                                                            output_device=args.rank,
                                                            find_unused_parameters=True)

    optimizer = AdamW(filter(lambda p: p.requires_grad, colcondenser.parameters()), lr=args.lr, eps=1e-8)
    optimizer.zero_grad()
    ...
    for batch_idx, BatchSteps in zip(range(start_batch_idx, args.maxsteps), reader):
        ...
        for queries, passages in BatchSteps:
            with amp.context():
                scores = colcondenser(queries, passages).view(2, -1).permute(1, 0)
        ...
        amp.step(colcondenser, optimizer)
        ...
        if args.rank < 1:
            ...
            manage_checkpoints(args, colcondenser, optimizer, batch_idx+1)
